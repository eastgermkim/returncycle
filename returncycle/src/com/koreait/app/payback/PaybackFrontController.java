...
public class PaybackFrontController extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		if(command.equals("/payback/PaybackOk.py")) {
			try {
				forward = new PaybackOkAction().execute(req, resp);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else if(command.equals("/payback/Payback_result.py")) {
			forward = new ActionForward();
			forward.setRedirect(false);
			forward.setPath("/app/payback/Payback_result.jsp");
		}else {
			forward = new ActionForward();
			forward.setRedirect(false);
