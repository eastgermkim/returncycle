...
public class DeliverFrontController extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		...
		if(command.equals("/Deliver/DeliverGo.do")) {
			
			try {
				forward = new DeliverGo().execute(req, resp);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else if(command.equals("/deliver/DeliverResult.do")) {
			forward = new ActionForward();
			forward.setRedirect(false);
			forward.setPath("/app/deliver/deliverResult.jsp");
		}else {
			forward = new ActionForward();
			forward.setRedirect(false);
